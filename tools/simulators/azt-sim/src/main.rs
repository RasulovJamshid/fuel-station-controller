...
async fn main() -> anyhow::Result<()> {
    ...
    tracing::info!("AZT 2.0 Simulator starting");
        ...
        site.active_positions()
            ...
            .map(|fp| {
                let nozzles: Vec<NozzleCfg> = fp
                    ...
                    .map(|n| NozzleCfg {
                        index: n.index,
                        // Site config carries soum; the TRK's registers are wire
                        // units (1 wire = 10 soum — real pumps print "100" for
                        // 100 000 soum). Same convention as the service side.
                        price: n.price / 10,
                    })
                    .collect();
