...
mod tests {
    ...
    fn frame_endings() {
        ...
        assert_eq!(f, vec![0x52, 0x30, 0x01, 0x01, 0x04, 0xE7, 0x5F, 0x03, 0xFA]);
        let f = build_frame(&[0x52, 0x30, 0x01, 0x01, 0x08]);
        assert_eq!(f, vec![0x52, 0x30, 0x01, 0x01, 0x08, 0xE7, 0x5A, 0x03, 0xFA]);
        let f = build_frame(&[0x52, 0x30, 0x01, 0x01, 0x05]);
        assert_eq!(f, vec![0x52, 0x30, 0x01, 0x01, 0x05, 0x26, 0x9F, 0x03, 0xFA]);
        let f = build_frame(&[0x53, 0x30, 0x01, 0x01, 0x08]);
        assert_eq!(f, vec![0x53, 0x30, 0x01, 0x01, 0x08, 0xDA, 0x9A, 0x03, 0xFA]);
        let f = build_frame(&[0x50, 0x30, 0x01, 0x01, 0x08]);
        assert_eq!(f, vec![0x50, 0x30, 0x01, 0x01, 0x08, 0x9E, 0x9A, 0x03, 0xFA]);
        let f = build_frame(&[0x51, 0x30, 0x01, 0x01, 0x08]);
        assert_eq!(f, vec![0x51, 0x30, 0x01, 0x01, 0x08, 0xA3, 0x5A, 0x03, 0xFA]);
    }
}
